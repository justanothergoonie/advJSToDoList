...
var updateItemCount = function updateItemCount() {
  leftCount.innerText = document.querySelectorAll('li:not(.finished').length, doneCount.innerText = document.querySelectorAll('li.finished').length, importCount.innerText = document.querySelectorAll('li.important').length;
};

updateItemCount();

var isFieldValid = function isFieldValid() {
  return '' !== item.value.trim() || (alert("I'm sorry Chris, I'm afraid I Can't do that... Dasiy, Dais.."), !1);
},
  ...
  if (isFieldValid()) {
    ...
    t.innerText = '❌', n.innerText = '❕', t.addEventListener('click', function () {
      e.classList.toggle('finished'), e.classList.contains('finished') ? t.innerText = '✅' : t.innerText = '❌', updateItemCount();
    }), n.addEventListener('click', function () {
      e.classList.toggle('important'), e.classList.contains('important') ? n.innerText = '❗' : n.innerText = '❕', updateItemCount();
    }), e.innerText = item.value, theList.appendChild(e), e.appendChild(t), e.appendChild(n), item.value = '', item.placeholder = 'Please Enter New Item', item.focus(), updateItemCount();
  }
},
    removeItems = function removeItems() {
  for (var e = document.querySelectorAll('.finished'), t = 0; t < e.length; t++) {
    e[t].remove();
  }
...
addBttn.addEventListener('click', addItem), window.addEventListener('keyup', function () {
